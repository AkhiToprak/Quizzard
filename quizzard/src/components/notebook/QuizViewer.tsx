...
export default function QuizViewer({ notebookId, setId, title, initialQuestions, assignedSectionId }: QuizViewerProps) {
  ...
  const correctCount = Array.from(answers.entries()).filter(([idx, ans]) => questions[idx]?.correctIndex === ans).length;
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      else if (e.code === 'Digit1' || e.code === 'KeyA') { e.preventDefault(); selectAnswer(0); }
      else if (e.code === 'Digit2' || e.code === 'KeyB') { e.preventDefault(); selectAnswer(1); }
      else if (e.code === 'Digit3' || e.code === 'KeyC') { e.preventDefault(); selectAnswer(2); }
      else if (e.code === 'Digit4' || e.code === 'KeyD') { e.preventDefault(); selectAnswer(3); }
      else if (e.code === 'KeyH') { e.preventDefault(); setShowHint(v => !v); }
    };
  ...
  const saveEdit = async () => {
    if (!editingId) return;
    try {
      const res = await fetch(`/api/notebooks/${notebookId}/quiz-sets/${setId}/questions/${editingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const deleteQuestion = async (questionId: string) => {
    ...
    try {
      await fetch(`/api/notebooks/${notebookId}/quiz-sets/${setId}/questions/${questionId}`, { method: 'DELETE' });
      const newQuestions = questions.filter(q => q.id !== questionId);
      setQuestions(newQuestions);
      if (currentIndex >= newQuestions.length) setCurrentIndex(Math.max(0, newQuestions.length - 1));
    } catch { /* silent */ }
  };
        ...
        <div style={{
          ...
          {[
            ...
            <div key={stat.label} style={{
              ...
              <div style={{ fontSize: '11px', color: 'rgba(237,233,255,0.35)', marginTop: '2px' }}>{stat.label}</div>
            </div>
          ))}
        </div>
    ...
    <div ref={containerRef} style={{
      ...
      {editingId === question?.id ? (
        ...
        <div style={{
          ...
          <textarea value={editCorrectExplanation} onChange={e => setEditCorrectExplanation(e.target.value)} rows={2} style={textareaStyle} />
          <label style={labelStyle}>Wrong Explanation (optional)</label>
          <textarea value={editWrongExplanation} onChange={e => setEditWrongExplanation(e.target.value)} rows={2} style={textareaStyle} />
        ...
        <div style={{
              ...
              return (
                ...
                >
                  <span style={{
                    ...
                    color: textColor,
                    background: showResult && isCorrectOption ? 'rgba(74,222,128,0.15)' :
                      showResult && (isSelected || reviewSelected) ? 'rgba(252,165,165,0.15)' : 'transparent',
                  }}>
          ...
          {showHint && question.hint && (
            <div style={{
              ...
              fontSize: '13px', color: 'rgba(251,191,36,0.8)',
          ...
          {isAnswered && (
            <div style={{
              ...
              <div style={{
                ...
                {isCorrect ? <><CheckCircle2 size={16} /> Correct!</> : <><XCircle size={16} /> Not quite</>}
              </div>
              <div style={{ fontSize: '13px', color: 'rgba(237,233,255,0.6)', lineHeight: 1.6 }}>
                {isCorrect
                  ? (question.correctExplanation || `The answer is ${question.options[question.correctIndex]}.`)
                  : (question.wrongExplanation || `The correct answer is ${question.options[question.correctIndex]}.`)
                }
              </div>
            </div>
          )}
              ...
              {answers.has(currentIndex) ? (
                <>
                  <div style={{
                    display: 'flex', alignItems: 'center', gap: '8px',
                    fontSize: '14px', fontWeight: 700, marginBottom: '6px',
                    color: answers.get(currentIndex) === question.correctIndex ? '#4ade80' : '#fca5a5',
                  }}>
                    {answers.get(currentIndex) === question.correctIndex
                ...
                <div style={{ fontSize: '13px', color: 'rgba(237,233,255,0.4)' }}>
                  You skipped this question. The correct answer is <strong style={{ color: '#4ade80' }}>{question.options[question.correctIndex]}</strong>.
                </div>
              )}
            </div>
          )}
      ...
      <div style={{
        ...
        {currentIndex === questions.length - 1 && mode === 'quiz' ? (
          ...
          <NavButton onClick={next} disabled={currentIndex === questions.length - 1} title="Next (→)">
...
const labelStyle: React.CSSProperties = {
  fontSize: '11px', color: 'rgba(237,233,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em',
};
...
function NavButton({ onClick, disabled, title, children, highlight }: {
  onClick: () => void; disabled?: boolean; title: string; children: React.ReactNode; highlight?: boolean;
}) {
  const [hovered, setHovered] = useState(false);
  return (
    <button
      ...
      style={{
        ...
        background: highlight
          ...
          : hovered && !disabled ? 'rgba(140,82,255,0.15)' : 'rgba(140,82,255,0.06)',
