...
export default function MaterialPicker({ notebookId, onSelect, onClose }: MaterialPickerProps) {
    ...
    async function load() {
      ...
      try {
        ...
        if (flashRes.success && flashRes.data) {
          setFlashcardSets(
            flashRes.data.map((f: { id: string; title: string }) => ({ id: f.id, title: f.title }))
          );
        }
        if (quizRes.success && quizRes.data) {
          setQuizSets(
            quizRes.data.map((q: { id: string; title: string }) => ({ id: q.id, title: q.title }))
          );
        }
        if (docRes.success && docRes.data) {
          setDocuments(
            docRes.data.map((d: { id: string; fileName: string }) => ({
              id: d.id,
              title: d.fileName,
            }))
          );
  ...
  const getItems = (): { items: MaterialItem[]; type: SelectedMaterial['type'] } => {
    switch (activeTab) {
      case 'pages':
        return { items: pages, type: 'page' };
      case 'flashcard_sets':
        return { items: flashcardSets, type: 'flashcard_set' };
      case 'quiz_sets':
        return { items: quizSets, type: 'quiz_set' };
      case 'documents':
        return { items: documents, type: 'document' };
    }
  };
